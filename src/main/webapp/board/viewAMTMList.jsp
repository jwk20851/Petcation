...
<body>
	<div style="height: 100%; width: 100%;">
		<div id="id01" class="modal">
			<div id="body-content">
				...
				<form name="amtm" class="modal-content" action="deleteAMTM.bo">
					<div class="head">
						<h1 style="font-size: 3vw; text-align: center;">1대1문의내역</h1>
					</div>
					...
					<div class="out_container">
						...
						<div class="container" style="padding: 5%">
							...
							<c:if test="${empty viewAMTMList}">
								<table width="100%" border="1" cellpadding="0" cellspacing="0">
									<tr>
										<td align="center">게시판에 저장된 글이 없습니다. <input type="button"
											value="글작성" OnClick="fn_insert_form()"></td>
									<tr>
								</table>
							</c:if>
								...
								<table border="1" width="100%" cellpadding="0" cellspacing="0"
									...
									<tr height="30">
										...
										<td align="center" width="50"><input type="checkbox"
											id="allCheck" name="allCheck" Onclick="checkAll()"></td>
										<td align="center" width="250">글제목</td>
										<td align="center" width="50">작성자</td>
										<td align="center" width="100">답변여부</td>
										<td align="center" width="150">작성일</td>
									</tr>
									<c:set var="number" value="${pageVO.number}"></c:set>

									<c:forEach var="amtm" items="${articleList}">

										<tr height="30">
											<td align="center" width="50">${number}</td>
											<c:set var = "number" value = "${number - 1 }"></c:set>	<!-- 번호 -->
											<td align="center"><input value="${amtm.num}"
												type="checkbox" name="delete1" />${amtm.checkbox}</td>
											<td align="center" width="50"><a
												href="amtm_content.bo?num=${amtm.num}">${amtm.title}</a></td>
											<td align="center" width="50">작성자</td>
											<c:if test="${empty amtm.answer}">
												<td align="center" width="100">대기중</td>
											</c:if>
											<c:if test="${not empty amtm.answer}">
												<td align="center" width="100">답변완료</td>
											</c:if>

											<td align="center" width="100"><fmt:formatDate
													pattern="yyyy-MM-dd " value="${amtm.reg_date}" /></td>
										</tr>
									</c:forEach>
								...
								<table width="100%" align="center">
									<tr>
										<td colspan=2 align="right"><select name="cars" id="cars"
											...
											<form class="example" action="/action_page.php"
												...
												<input type="text" placeholder="내용 또는 제목" name="search1"
													style="height: 40px;">
									...
									<tr>
										<td colspan=2 align="right"><input type="button"
											value="글삭제" style="width: 60px; height: 40px; float: right;"
											OnClick="recont()"></td>
									</tr>
								</table>
							</c:if>
							...
							<c:if test="${pageVO.count > 0}">
								...
								<c:if test="${pageVO.startPage > 10}">
									<a
										href="amtmList.bo?pageNum=<%-- <%= startPage - 10 %> --%>${pageVO.startPage - 10}">[이전]</a>
								</c:if>

								<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}"
									var="i">
									<a href="amtmList.bo?pageNum=<%-- <%= i %> --%>${i}">[<%-- <%= i %> --%>${i}]</a>
									<!-- 해당번호를 클릭하면 클린한 페이지로 이동한다. -->
								</c:forEach>

								<c:if test="${pageVO.endPage < pageVO.pageCount}">
									<a
										href="amtmList.bo?pageNum=<%-- <%= startPage + 10 %> --%>${pageVO.startPage + 10}">[다음]</a>
								</c:if>
	...
	<script>
		...
		function checkAll() {
			...
			var delete1 = document.amtm.delete1;
