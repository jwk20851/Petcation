...
<script>
	function fn_insert_form() {
		location.href = "mtm_insert_form.bo";
	}
</script>
					...
					<div class="out_container">
						<div class="sidebar">
							<div class="sidenav">
								...
								<a href="viewNoticeList.bo">공지사항</a> <a href="faqList.bo">FAQ</a>
								<a href="mtm_insert_form.bo">1대1 문의하기</a> <a
									href="viewMTMList.bo">1대1 문의내역</a>
							</div>
						</div>
						<div class="container" style="padding: 5%">

							<c:if test="${empty viewMTMList}">
							...
							<c:if test="${not empty articleList}">
								<table border="1" width="100%" cellpadding="0" cellspacing="0"
									...
									<c:forEach var="mtm" items="${articleList}">

										<tr height="30">
											<td align="center" width="50">${number}</td>
											<c:set var = "number" value = "${number - 1 }"></c:set>	<!-- 번호 -->
											<td align="center" width="50"><a href="mtm_content.bo?num=${mtm.num}">${mtm.title}</a></td>
											<c:if test="${empty mtm.answer}">
												<td align="center" width="100">대기중</td>
											</c:if>
											<c:if test="${not empty mtm.answer}">
												<td align="center" width="100">완료</td>
											</c:if>

											<td align="center" width="150"><fmt:formatDate
													pattern="yyyy-MM-dd " value="${mtm.reg_date}" /></td>
										</tr>
							...
							<c:if test="${pageVO.count > 0}">
								...
								<c:if test="${pageVO.startPage > 10}">
									<a
										href="viewMTMList.bo?pageNum=<%-- <%= startPage - 10 %> --%>${pageVO.startPage - 10}">[이전]</a>
								</c:if>

								<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}"
									var="i">
									<a href="viewMTMList.bo?pageNum=<%-- <%= i %> --%>${i}">[<%-- <%= i %> --%>${i}]</a>
									<!-- 해당번호를 클릭하면 클린한 페이지로 이동한다. -->
								</c:forEach>

								<c:if test="${pageVO.endPage < pageVO.pageCount}">
									<a
										href="viewMTMList.bo?pageNum=<%-- <%= startPage + 10 %> --%>${pageVO.startPage + 10}">[다음]</a>
								</c:if>
