...
public class BookMarkListController {
	...
	@RequestMapping("/bookMark_list.bo")
	public ModelAndView viewBookMarkList(HttpServletRequest request) {
		List<BoardVO> manageBookmark = bookMarkListService.getBookMarkList();
		...
		int totalCount = manageBookmark.size();
		int number = 0;
		
		List<BoardVO> articleList = null;
		
		if(totalCount > 0) {
			articleList = bookMarkListService.getArticleList(startRow, pageSize);
		}
		...
		mav.setViewName("manageBookmark");
		mav.addObject("manageBookmark", manageBookmark);
