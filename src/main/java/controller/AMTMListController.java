...
@Controller
public class AMTMListController {
	@Autowired
	private ViewAMTMService viewAMTMService;
	
	@RequestMapping("/amtmList.bo")
	public ModelAndView MTMList(HttpServletRequest request) {
		List<BoardVO> viewAMTMList = viewAMTMService.getAMTMList();
		...
		int totalCount = viewAMTMList.size();
		int number = 0;
		
		List<BoardVO> articleList = null;
		
		if(totalCount > 0) {
			articleList = viewAMTMService.getArticleList(startRow, pageSize);
		}
		...
		mav.setViewName("viewAMTMList");
		mav.addObject("viewAMTMList", viewAMTMList);
