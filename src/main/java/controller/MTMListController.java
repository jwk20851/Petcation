...
@Controller
public class MTMListController {
	@Autowired
	private ViewMTMService viewMTMService;
	
	@RequestMapping("/viewMTMList.bo")
	public ModelAndView MTMList(HttpServletRequest request) {
		List<BoardVO> viewMTMList = viewMTMService.getMTMList();
		...
		int totalCount = viewMTMList.size();
		int number = 0;
		
		List<BoardVO> articleList = null;
		
		if(totalCount > 0) {
			articleList = viewMTMService.getArticleList(startRow, pageSize);
		}
		...
		mav.setViewName("viewMTMList");
		mav.addObject("viewMTMList", viewMTMList);
