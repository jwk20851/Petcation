...
public class FAQListController {
	...
	public ModelAndView FAQList(HttpServletRequest request) {
		List<FAQVO> viewFAQList = fAQService.getFAQList();
		...
		int totalCount = viewFAQList.size();
		int number = 0;
		
		List<FAQVO> articleList = null;
		
		if(totalCount > 0) {
			articleList = fAQService.getArticleList(startRow, pageSize);
		}
		...
		mav.setViewName("viewFAQList");
		mav.addObject("viewFAQList", viewFAQList);
